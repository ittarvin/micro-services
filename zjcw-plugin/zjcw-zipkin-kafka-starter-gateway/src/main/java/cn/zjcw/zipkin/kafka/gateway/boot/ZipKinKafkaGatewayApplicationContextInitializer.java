 ...
 * @since 2022-04-13
 */
public class ZipKinKafkaGatewayApplicationContextInitializer implements  ApplicationContextInitializer<ConfigurableApplicationContext>, Ordered {

    private static final Logger logger = LoggerFactory.getLogger(ZipKinKafkaGatewayApplicationContextInitializer.class);
    ...
    public void initialize(ConfigurableApplicationContext configurableApplicationContext) {
        ...
        if (!environment.getProperty(ZipkinKafkaPropertySourcesConstants.ZIPKIN_KAFKA_BOOTSTRAP_ENABLED, Boolean.class, false)) {
            logger.debug("Zipkin 2 Kafka Gateway bootstrap config is not enabled for context {}, see property: ${{}}",
                    configurableApplicationContext, ZipkinKafkaPropertySourcesConstants.ZIPKIN_KAFKA_BOOTSTRAP_ENABLED);
            return;
        }

        logger.debug("Zipkin 2 Kafka Gateway bootstrap config is enabled for context {}", configurableApplicationContext);
